...
export class UserService {
  ...
  async login(user: UserI): Promise<string> {
    ...
    if(foundUser) {
      const passwordsMatching: boolean = await this.authService.comparePasswords(user.password, foundUser.password);

      if (passwordsMatching === true) {
        const payload: UserI = await this.findOne(foundUser.id);
        return this.authService.generateJwt(payload);
      } else {
        throw new HttpException('Login was not successfull, wrong credentils', HttpStatus.UNAUTHORIZED);
      }
    } else {
